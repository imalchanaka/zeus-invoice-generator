// Legacy non-module UI script for older static HTML version
// Renamed to avoid collision with the React App module
...
function generateInvoiceNumber() {
  ...
  localStorage.setItem(
    STORAGE_SEQ_KEY,
    JSON.stringify({ date: dateStr, lastSeq: seq })
  );
  return invoiceNo;
}
...
function addRow(data = null) {
  ...
  tr.innerHTML = `
        <td class="border border-gray-300 px-4 py-2 relative">
            ...
            <textarea placeholder="Description of work..." class="w-full border-none focus:ring-0 p-0 text-sm outline-none bg-transparent">${
              data?.description || ""
            }</textarea>
        </td>
        <td class="border border-gray-300 px-4 py-2">
            <input type="number" step="any" value="${
              data?.qty || ""
            }" class="calc-trigger qty-input w-full border-none focus:ring-0 p-0 text-center text-sm outline-none bg-transparent">
        </td>
        <td class="border border-gray-300 px-4 py-2">
            <input type="text" value="${
              data?.unit || ""
            }" class="w-full border-none focus:ring-0 p-0 text-center text-sm outline-none bg-transparent">
        </td>
        <td class="border border-gray-300 px-4 py-2">
            <input type="number" step="0.01" value="${
              data?.rate || ""
            }" class="calc-trigger rate-input w-full border-none focus:ring-0 p-0 text-center text-sm outline-none bg-transparent">
        </td>
